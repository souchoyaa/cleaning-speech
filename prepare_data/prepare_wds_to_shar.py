        ...
        try:
            for member in tf:
                ...
                if ext in SIDECAR_SUFFIXES:
                    ...
                    # Some datasets ship sidecars as `<id>.metadata.json` while
                    # audio is `<id>.flac`; strip the trailing `.metadata` so
                    # sidecar and audio stems line up.
                    if ext == ".json" and stem.endswith(".metadata"):
